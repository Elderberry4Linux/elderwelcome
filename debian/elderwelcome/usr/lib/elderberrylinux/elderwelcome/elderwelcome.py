...
NORUN_FLAG = os.path.expanduser("~/.elderberrylinux/elderwelcome/norun.flag")

# i18n
gettext.install("elderwelcome", "/usr/share/elderberrylinux/locale")
from locale import gettext as _
locale.bindtextdomain("elderwelcome", "/usr/share/elderberrylinux/locale")
locale.textdomain("elderwelcome")
...
class ElderWelcome():
    ...
    def __init__(self):
        builder = Gtk.Builder()
        builder.set_translation_domain("elderwelcome")
        builder.add_from_file('/usr/share/elderberrylinux/elderwelcome/elderwelcome.ui')

        window = builder.get_object("main_window")
        window.set_icon_name("elderwelcome")
        window.set_position(Gtk.WindowPosition.CENTER)
        window.connect("destroy", Gtk.main_quit)

        with open("/etc/elderberrylinux/info") as f:
        ...
        # Action buttons
        builder.get_object("button_forums").connect("clicked", self.visit, "http://forum.elderberrylinux.org")
        builder.get_object("button_documentation").connect("clicked", self.visit, "http://elderberrylinux.org/documentation.php")
        builder.get_object("button_contribute").connect("clicked", self.visit, "http://elderberrylinux.org/getinvolved.php")
        builder.get_object("button_irc").connect("clicked", self.visit, "irc://irc.spotchat.org/elderberrylinux-help")
    ...
    def on_button_toggled(self, button):
        if button.get_active():
            ...
            os.system("mkdir -p ~/.elderberrylinux/elderwelcome")
